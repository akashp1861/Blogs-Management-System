...
if (isset($_GET['sno'])) {
    ...
    $authorQuery = "SELECT username FROM contact WHERE sno = $userID";
    ...
    if ($authorResult && mysqli_num_rows($authorResult) == 1) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<div class="mt-3">';
                echo '<a href="edit_blog.php?id=' . $blogID . '" class="btn btn-warning me-2">Edit</a>';
                echo '<a href="delete_blog.php?id=' . $blogID . '" class="btn btn-danger" onclick="return confirm(\'Are you sure you want to delete this blog?\')">Delete</a>';
                echo '</div>';
